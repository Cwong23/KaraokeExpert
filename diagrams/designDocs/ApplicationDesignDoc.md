...
### Design Explanation
#### Frontend
The frontend is built with React and Vite. Authentication is handled through a login/sign up page, and on successful authentication, the user is taken to the home page, which displays their uploaded songs and an option to upload a new one. Once the user clicks "upload song", the website navigates to the song preview page where they can view their chosen song's lyrics once the backend returns the lyrics and instrumental. The page also allows users to play the lyrics in sync with the instrumental, creating a karaoke-style experience where the lyrics follow along with the music in real time.
